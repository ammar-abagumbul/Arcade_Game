...
class Ghost : public GameObject {
  ...
  /**
   * Follows a target object on the game map by setting the direction of the ghost to the direction
   * return by the `FindPath()` method
   *
   ...
   * @param map The game map.
   */
  void ChangeDirection(ObjectState playerState, ObjectState ghostState, char **map);

  /**
   * Picks a random direction for the ghost to move on the game map at an intersection.
   *
   * @param map The game map.
   */
  ...
  /**
   * Returns the available turns for the ghost on the game map. Reversing direction
   * is often not considered a valid turn unless the game logic calls `ReverseDirection()`
   * explicitly
   *
   * @param map The game map.
  ...
  /**
   * Finds a path from the current ghost position to the specified coordinates on the game map.
   * Uses Breadth-first search algoirthm.
   *
  ...
  private:
  ObjectState targetState; // The point that the ghost targets when in SCATTER mode
  Mode mode;
};
